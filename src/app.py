...
url = 'https://raw.githubusercontent.com/Mpox-Predictor/Mpox-csv/main/forecast_df.csv'
Ldata = pd.read_csv(url,index_col=0)
...
"""
Ldata1 = pd.DataFrame()
Ldata1 ['date'] = Ldata1.index
Ldata1 ['Countries'] = Ldata1.iloc[:1]
Ldata1 ['Cases'] = Ldata ['United States']
Ldata1 ['Codes'] = Ldata1 ['Countries'].apply(get_continent)
Ldata1['Country Code'] = Ldata1 ['Codes'].apply(lambda x: x[0] + x[1] + x[2])
Ldata1 = Ldata1.set_index('date')
Ldata1 = Ldata1[Ldata1['Country Code'] !="Unk"]
Ldata1.drop('Codes',axis = 1, inplace = True)
"""
...
xs = 12, sm = 12, md = 4, lg = 4, xl = 4, style = {'padding':'12px 12px 12px 12px'}, className = "card-style")
